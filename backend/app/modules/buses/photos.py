...
from app.infrastructure.storage import presigned_get_url, upload_bytes

ALLOWED_CONTENT_TYPES: dict[str, str] = {"image/jpeg": "jpg", "image/png": "png"}
THUMBNAIL_SIZE = (320, 240)


def _pillow_format(content_type: str) -> Literal["JPEG", "PNG"]:
...
def process_and_upload(content_type: str, data: bytes) -> tuple[str, str]:
    ...
    if content_type not in ALLOWED_CONTENT_TYPES:
        raise UnprocessableEntityError("Only JPEG and PNG images are allowed")

    try:
        image = Image.open(io.BytesIO(data))
        image.verify()  # detects truncated / non-image payloads
    except (UnidentifiedImageError, OSError) as exc:
        raise UnprocessableEntityError("Uploaded file is not a valid image") from exc
